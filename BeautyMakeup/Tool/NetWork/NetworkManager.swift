...
func NetworkRequest<T: HandyJSON>(_ target: TargetType, needShowFailAlert: Bool = true, modelType: T.Type, successCallback: @escaping RequestModelSuccessCallback<T>, failureCallback: RequestCallback? = nil) -> Cancellable? {
    return NetWorkRequest(target, needShowFailAlert: needShowFailAlert) { (respModel: SuccessResponse) in
        if let _model = T.deserialize(from: respModel.dataJSON) {
            ...
            CocoaLog.debug("模型解析错误 ------- \(T.Type.self) ----\n\n \(respModel.dataJSON ?? [:])")
        }
...
/// - Returns: 取消当前网络请求Cancellable实例
@discardableResult
func NetWorkRequest<T: HandyJSON>(_ target: TargetType, needShowFailAlert: Bool = true, modelType: [T].Type, successCallback:@escaping RequestModelsSuccessCallback<T>, failureCallback: RequestCallback? = nil) -> Cancellable? {
    return NetWorkRequest(target, needShowFailAlert: needShowFailAlert) { (respModel: SuccessResponse) in
...
/// - Returns: 取消当前网络请求Cancellable实例
@discardableResult
private func NetWorkRequest(_ target: TargetType, needShowFailAlert: Bool = true, successCallback:@escaping ((SuccessResponse) -> Void), failureCallback: RequestCallback? = nil) -> Cancellable? {
    // 判断网络状态
    guard UIDevice.isNetworkConnect else {
        errorHandler(error: ErrorResponse(code: NETWORK_ERROR, message: "The network is lost"), failure: failureCallback)
        return nil
    }
    ...
    return AuthProvider.request(MultiTarget(target)) { result in
        ...
        case let .success(response):
            do {
                ...
                errorHandler(error: ErrorResponse(code: JSON_SERIALIZATION_ERROR, message: "JSON SERIALIZATION ERROR"), failure: failureCallback)
            }
        case let .failure(error as NSError):
            failureCallback?(ErrorResponse(code: error.code, message: error.localizedDescription))
        }
    }
}
